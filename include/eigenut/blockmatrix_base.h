...
#ifndef H_EIGENUT_BLOCKMATRIX_BASE
#define H_EIGENUT_BLOCKMATRIX_BASE
...
#define EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM   ((t_block_rows_num > 0) ? t_block_rows_num : block_rows_num_)
#define EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM   ((t_block_cols_num > 0) ? t_block_cols_num : block_cols_num_)

#define     EIGENUT_DEFINE_BLOCK_MATRIX_CONSTRUCTORS(class_name) \
            class_name (const std::ptrdiff_t  block_rows_num = MatrixBlockSizeType::UNDEFINED,\
                ...
                : EIGENUT_PARENT_CLASS_SHORTHAND(block_rows_num, block_cols_num) {};\
    ...
    class EIGENUT_VISIBILITY_ATTRIBUTE BlockMatrixAccessBase
    {
        public:
            ...
            typedef Eigen::Block< EIGENUT_DYNAMIC_MATRIX( Scalar ) > DynamicMatrixBlock;

            /// Shorthand for Eigen block
            typedef const Eigen::Block< const EIGENUT_DYNAMIC_MATRIX( Scalar ) > ConstDynamicMatrixBlock;
        ...
        public:
            ...
            {
                return(EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM * num_blocks_vert_);
            }
            ...
            {
                return(EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM * num_blocks_hor_);
            }
            ...
            {
                matrix_.resize( num_blocks_vert*EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM,
                                num_blocks_hor*EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM);
                finalize();
            }
            ...
            void setZero(   const std::ptrdiff_t   num_blocks_vert,
                            const std::ptrdiff_t   num_blocks_hor)
            {
                matrix_.setZero(num_blocks_vert*EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM,
                                num_blocks_hor*EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM);
                finalize();
            }
                                        ...
                                        const std::ptrdiff_t index_num_rows)
            {
                return (DynamicMatrixBlock (matrix_.derived(),
                                            index_row_first * EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM,
                                            index_col       * EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM,
                                            index_num_rows  * EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM,
            ...
            {
                return (DynamicMatrixBlock( matrix_.derived(),
                                            index_row       * EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM,
                                            index_col_first * EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM,
                                            EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM,
                                            index_num_cols  * EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM));
            }
            ...
            {
                return(selectRows(matrix_, EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM, row_in_a_block));
            }
            ...
            {
                return(EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM);
            }
            ...
            {
                return(EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM);
            }
            ...
            {
                EIGENUT_ASSERT(  (MatrixBlockSizeType::DYNAMIC != t_block_rows_num) || (block_rows_num > 0),
                                    "Block dimension must be strictly positive.");
                EIGENUT_ASSERT(  (MatrixBlockSizeType::DYNAMIC != t_block_cols_num) || (block_cols_num > 0),
                                    "Block dimension must be strictly positive.");
        ...
        protected:
                                        ...
                                        const std::ptrdiff_t  block_cols_num)
            {
                ...
                {
                    EIGENUT_ASSERT(  (MatrixBlockSizeType::UNDEFINED == block_rows_num),
                ...
                {
                    EIGENUT_ASSERT(  (MatrixBlockSizeType::UNDEFINED == block_cols_num),
                                        "Matrix block is static, but nonzero dimension size is passed to the constructor.");
            ...
            void finalize()
            {
                ...
                {
                    EIGENUT_ASSERT(  (EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM > 0),
                                        "Wrong number of rows.");

                    EIGENUT_ASSERT(  (matrix_.rows() % EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM == 0),
                                        "Vertical dimension of the matrix is not a multiple of the corresponding block dimension.");

                    num_blocks_vert_ = matrix_.rows() / EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM;
                }

                if (    (t_block_cols_num != MatrixBlockSizeType::DYNAMIC)
                        || (block_cols_num_ != MatrixBlockSizeType::UNDEFINED))
                {
                    EIGENUT_ASSERT(  (EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM > 0),
                                        "Wrong number of cols.");

                    EIGENUT_ASSERT(  (matrix_.cols() % EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM == 0),
                                        "Horizontal dimension of the matrix is not a multiple of the corresponding block dimension.");

                    num_blocks_hor_ = matrix_.cols() / EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM;
                }
            }
    };


#define EIGENUT_PARENT_CLASS_SHORTHAND   BlockMatrixAccessBase<t_MatrixType, t_block_rows_num, t_block_cols_num>
    /**
    ...
    template<   typename t_MatrixType,
                int t_block_rows_num,
                int t_block_cols_num>
    class EIGENUT_VISIBILITY_ATTRIBUTE BlockMatrixSizeSpecificBase : public EIGENUT_PARENT_CLASS_SHORTHAND
    {
        public:
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::DecayedRawMatrix DecayedRawMatrix;
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::Scalar           Scalar;


            /// Shorthand for Eigen block
            typedef Eigen::Block<   EIGENUT_DYNAMIC_MATRIX( Scalar ),
                                    t_block_rows_num,
                                    t_block_cols_num> StaticMatrixBlock;

            /// Shorthand for Eigen block
            typedef const Eigen::Block< const EIGENUT_DYNAMIC_MATRIX( Scalar ),
                                        t_block_rows_num,
        ...
        protected:
            ...
            EIGENUT_DEFINE_BLOCK_MATRIX_CONSTRUCTORS(BlockMatrixSizeSpecificBase)
    };
#undef EIGENUT_PARENT_CLASS_SHORTHAND


#define EIGENUT_CODE_GENERATOR\
        public:\
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::DecayedRawMatrix        DecayedRawMatrix;\
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::DynamicMatrixBlock      DynamicMatrixBlock;\
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::ConstDynamicMatrixBlock ConstDynamicMatrixBlock;\
...
#define EIGENUT_PARENT_CLASS_SHORTHAND   BlockMatrixAccessBase<  t_MatrixType, \
     ...
     */
    template<typename t_MatrixType>
        class EIGENUT_VISIBILITY_ATTRIBUTE BlockMatrixSizeSpecificBase<  t_MatrixType,
                                                                            MatrixBlockSizeType::DYNAMIC,
                                                                            MatrixBlockSizeType::DYNAMIC>
            : public EIGENUT_PARENT_CLASS_SHORTHAND
    {
        EIGENUT_CODE_GENERATOR
    };
#undef EIGENUT_PARENT_CLASS_SHORTHAND
...
#undef EIGENUT_PARENT_CLASS_SHORTHAND
#undef EIGENUT_CODE_GENERATOR


#define EIGENUT_PARENT_CLASS_SHORTHAND     BlockMatrixAccessBase<t_MatrixType, 1, 1>
    /**
     * @brief Extra layer for handling of specific sizes of blocks using partial
     * template specialization
     *
     ...
     */
    template<typename t_MatrixType>
    class EIGENUT_VISIBILITY_ATTRIBUTE BlockMatrixSizeSpecificBase<t_MatrixType, 1, 1> : public EIGENUT_PARENT_CLASS_SHORTHAND
    {
        public:
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::DecayedRawMatrix    DecayedRawMatrix;
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::Scalar              Scalar;
...
#define EIGENUT_PARENT_CLASS_SHORTHAND     BlockMatrixSizeSpecificBase<t_MatrixType, t_block_rows_num, t_block_cols_num>
    /**
    ...
    class EIGENUT_VISIBILITY_ATTRIBUTE BlockMatrixBase : public EIGENUT_PARENT_CLASS_SHORTHAND
    {
        protected:
            using EIGENUT_PARENT_CLASS_SHORTHAND::matrix_;
        ...
        public:
            ...
            EIGENUT_DEFINE_BLOCK_MATRIX_CONSTRUCTORS(BlockMatrixBase)
...
#define EIGENUT_PARENT_CLASS_SHORTHAND     BlockMatrixSizeSpecificBase<t_MatrixType, t_block_rows_num, t_block_cols_num>
    ...
    template<   typename t_MatrixType,
                int t_block_rows_num,
                int t_block_cols_num >
    class EIGENUT_VISIBILITY_ATTRIBUTE BlockMatrixBase<  t_MatrixType,
                                                            t_block_rows_num,
                                                            t_block_cols_num,
                                                            MatrixSparsityType::DIAGONAL > : public EIGENUT_PARENT_CLASS_SHORTHAND
    {
        public:
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::DecayedRawMatrix    DecayedRawMatrix;

            using EIGENUT_PARENT_CLASS_SHORTHAND::operator();
            using EIGENUT_PARENT_CLASS_SHORTHAND::getNumberOfRows;
            using EIGENUT_PARENT_CLASS_SHORTHAND::getNumberOfColumns;
            using EIGENUT_PARENT_CLASS_SHORTHAND::column;
            using EIGENUT_PARENT_CLASS_SHORTHAND::row;
            ...
            template<class t_DerivedOutput, class t_DerivedInput>
                void multiplyRight (Eigen::PlainObjectBase<t_DerivedOutput>     & result,
                                    const Eigen::MatrixBase<t_DerivedInput>     & matrix) const
            {
                EIGENUT_ASSERT(matrix.rows() == getNumberOfColumns(), "Size mismatch.");
                ...
                {
                    result.block(i*EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM, 0, EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM, num_cols).noalias() =
                        (*this)(i)
                        *
                        matrix.block(i*EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM, 0, EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM, num_cols);
                }
            }
            ...
            {
                EIGENUT_ASSERT(matrix.cols() == getNumberOfRows(), "Size mismatch.");
                ...
                {
                    result.block(0, i*EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM, num_rows, EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM).noalias() =
                        matrix.block(0, i*EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM, num_rows, EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM)
                        *
                        (*this)(i);
                }
            }
...
#define EIGENUT_PARENT_CLASS_SHORTHAND     BlockMatrixSizeSpecificBase<t_MatrixType, t_block_rows_num, t_block_cols_num>
    ...
    class EIGENUT_VISIBILITY_ATTRIBUTE BlockMatrixBase<  t_MatrixType,
                                                            t_block_rows_num,
                                                            t_block_cols_num,
                                                            MatrixSparsityType::LEFT_LOWER_TRIANGULAR> : public EIGENUT_PARENT_CLASS_SHORTHAND
    {
        public:
            typedef typename EIGENUT_PARENT_CLASS_SHORTHAND::DecayedRawMatrix    DecayedRawMatrix;
                                    ...
                                    const Eigen::MatrixBase<t_DerivedInput> & matrix) const
            {
                // slower
                /*
                EIGENUT_ASSERT(static_cast<std::ptrdiff_t>(eigen_matrix.rows()) == getNumberOfColumns(), "Size mismatch.");
                ...
                for(std::ptrdiff_t i = 0; i < num_blocks_hor_; ++i)
                {
                    result.block(i*EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM, 0, EIGENUT_BLOCKMATRIX_BLOCK_ROWS_NUM, num_cols).noalias() =
                        row(i, 0, i+1)
                        *
                        eigen_matrix.block(0, 0, EIGENUT_BLOCKMATRIX_BLOCK_COLS_NUM*(i+1), num_cols);
                }
                */
                result.noalias() = matrix_*matrix;
            }
...
#define EIGENUT_PARENT_CLASS_SHORTHAND     BlockMatrixSizeSpecificBase<t_MatrixType, 1, 1>
    /**
     * @brief Base class of a block matrix
     *
     * @tparam t_MatrixType     type of raw matrix
...
#undef EIGENUT_PARENT_CLASS_SHORTHAND
    ...
    template<   typename t_MatrixType,
                ...
                MatrixSparsityType::Type t_sparsity_type,
                class t_Derived>
        EIGENUT_DYNAMIC_MATRIX( typename TypeDecayed<t_MatrixType>::Type::Scalar )
    ...
    {
        EIGENUT_DYNAMIC_MATRIX( typename TypeDecayed<t_MatrixType>::Type::Scalar ) result;
        ...
        EIGENUT_VISIBILITY_ATTRIBUTE
            operator* ( const Eigen::MatrixBase<t_Derived> & eigen_matrix,
                        const BlockMatrixBase<  t_MatrixType,
                                                ...
                                                t_sparsity_type> & bm)
    {
        EIGENUT_DYNAMIC_MATRIX( typename Eigen::DenseBase<t_Derived>::Scalar ) result;
